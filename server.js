...
app.use((req, res) => {
    ...
    if (req.path.startsWith('/play/')) {
        ...
        if (videoId) {
            ...
            const playTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist}` : qTitle) : `Dengarkan Musik - AustinPlay`;
            const playDesc = `Dengarkan ${qTitle || 'lagu favoritmu'} di AustinPlay Web Music Player`;
    ...
    if (req.path.startsWith('/artist/')) {
        ...
        if (artistId) {
            ...
            const pageTitle = qName ? `${qName} (Artist) - AustinPlay` : `Artist - AustinPlay`;
            const pageDesc = qName ? `Dengarkan lagu & album terbaik dari ${qName} di AustinPlay` : `Dengarkan lagu & album dari artist favoritmu di AustinPlay`;
    ...
    if (req.path.startsWith('/album/')) {
        ...
        if (albumId) {
            ...
            const pageTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist} (Album) - AustinPlay` : `${qTitle} (Album) - AustinPlay`) : `Album - AustinPlay`;
            const pageDesc = qTitle ? `Dengarkan album ${qTitle} di AustinPlay` : `Dengarkan album favoritmu di AustinPlay`;
            const coverUrl = qCover || `https://www.gobox.my.id/file/R0ym4wqfznmp.png`;
...
// Only start a listener when run directly (local dev). On Vercel, this file
// is required by api/index.js and invoked as a serverless function instead.
