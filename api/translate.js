...
const handler = async (req, res) => {
    ...
    try {
        if (lines && Array.isArray(lines)) {
            ...
            return res.status(200).json({ status: true, result: { lines: translatedLines, creator: 'Austin' } });
        } else if (text) {
            const translatedText = await translateText(text, targetLang);
            return res.status(200).json({ status: true, result: { original: text, translated: translatedText, targetLang, creator: 'Austin' } });
        }
    } catch(e) {
