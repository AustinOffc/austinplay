var MP={
    init(){
        ...
        <div id="mini-player" class="hidden fixed left-4 right-4 sm:left-auto sm:right-6 sm:w-96 z-[160]" style="bottom:92px;transition:transform 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275);will-change:transform;transform:translate3d(0,150px,0);">
            <div id="mini-player-inner" onclick="FullPlayer.open()" class="rounded-2xl px-3.5 py-2.5 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition-all relative overflow-hidden bg-white/[0.12] backdrop-blur-2xl border border-white/20 shadow-2xl group">
                ...
                <div class="relative w-11 h-11 shrink-0 flex items-center justify-center z-10" onclick="FullPlayer.open(); if(typeof event !== 'undefined') event.stopPropagation();">
                    <svg class="w-11 h-11 -rotate-90 pointer-events-none absolute inset-0 z-10" viewBox="0 0 48 48">
                        ...
                        <circle id="mini-circle-progress" cx="24" cy="24" r="21" fill="none" stroke="#f3f3f1" stroke-width="2.5" stroke-dasharray="131.95" stroke-dashoffset="131.95" stroke-linecap="round" class="transition-all duration-150"></circle>
                    </svg>
                ...
                <div class="flex items-center gap-1.5 z-10 shrink-0">
                    ...
                    <button id="mini-like-btn" onclick="toggleCurrentLike(); if(typeof event !== 'undefined') event.stopPropagation();" class="text-[#a0a5b0] hover:text-white active:scale-90 p-1.5 cursor-pointer" title="Sukai Lagu">
    ...
    getTrackColors(track) {
        if (!track) return ['#e8e8e6', '#c7c7c5', '#a8a8a6', '#8a8a8a', '#6b6b6b'];
        ...
        hash = Math.abs(hash);
        var baseL = 55 + (hash % 30); // grayscale only — lightness varies per track, hue never does
        return [
            'hsl(0, 0%, ' + baseL + '%)',
            'hsl(0, 0%, ' + (((baseL + 10) % 85) + 10) + '%)',
            'hsl(0, 0%, ' + (((baseL + 20) % 85) + 10) + '%)',
            'hsl(0, 0%, ' + (((baseL + 75) % 85) + 10) + '%)',
            'hsl(0, 0%, ' + (((baseL + 65) % 85) + 10) + '%)'
        ];
    },
    applyColors(colors) {
