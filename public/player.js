// ============================================================
// AUSTINPLAY - CORE PLAYER (FULL FIX)
// ============================================================
...
const S={ht:[],sr:[],ar:[],hc:[],hcp:[],hca:[],sq:'',filter:'all',ct:null,pl:[],pi:-1,ps:'',ip:false,il:false,rm:'all',isShuffle:false,currentAccentColor:'#f3f3f1',autoNext:true,iv:null,pt:0,pd:0,at:'home',ld:{type:'none',lines:[]},cli:-1,lo:false,lyricOffset:0,playbackRate:1.0,sleepSecondsLeft:0,sleepEndWithTrack:false,volume:1.0,lastVolume:1.0};
try{S.playbackRate=parseFloat(localStorage.getItem('austin_playback_rate'))||1.0;}catch(e){S.playbackRate=1.0;}
try{var storedAutoNext = localStorage.getItem('austin_auto_next');if(storedAutoNext!==null){S.autoNext = storedAutoNext==='true';}}catch(e){}
...
function updateOG(title,image){
    var t=document.querySelector('meta[property="og:title"]');if(!t){t=document.createElement('meta');t.setAttribute('property','og:title');document.head.appendChild(t);}t.setAttribute('content',title+' | AustinPlay');
    ...
    document.title=title+' - AustinPlay';
}
...
try {
    var storedVol = parseFloat(localStorage.getItem('austin_volume'));
...
function applyVolume(vol) {
    ...
    S.volume = vol;
    if (AU) AU.volume = vol;
    try { localStorage.setItem('austin_volume', String(vol)); } catch(e){}
    updateVolumeUI();
}
...
function UB(){
    ...
    var accent = S.currentAccentColor || '#f3f3f1';
...
function updateOG(title, cover, artist) {
    if (title && cover) {
        ...
        var docTitle = fullTitle + ' | AustinPlay';
        var description = 'Dengarkan ' + fullTitle + ' di AustinPlay';
        ...
        document.title = 'AustinPlay';

        setMetaTag('og:title', 'AustinPlay', true);
        setMetaTag('og:description', 'AustinPlay - Web Music Player', true);
        setMetaTag('og:image', defaultCover, true);
        setMetaTag('og:image:width', '600', true);
        setMetaTag('og:image:height', '600', true);
        setMetaTag('og:url', location.href, true);
        setMetaTag('twitter:card', 'summary_large_image', false);
        setMetaTag('twitter:title', 'AustinPlay', false);
        setMetaTag('twitter:description', 'AustinPlay - Web Music Player', false);
...
function updateOGForArtist(artistName, coverUrl) {
    ...
    var docTitle = name + ' - Artist | AustinPlay';
    var description = 'Dengarkan lagu dan album terbaik dari ' + name + ' di AustinPlay';
...
function updateOGForAlbum(albumTitle, coverUrl, artistName) {
    ...
    var docTitle = fullTitle + ' - Album | AustinPlay';
    var description = 'Dengarkan album ' + fullTitle + ' di AustinPlay';
    var cover = (coverUrl && coverUrl !== FI) ? coverUrl : 'https://www.gobox.my.id/file/R0ym4wqfznmp.png';
...
function toggleAutoNext(){
    ...
    try { localStorage.setItem('austin_auto_next', S.autoNext); } catch(e) {}
...
function getLikedSongs(){
    try{return JSON.parse(localStorage.getItem('austin_liked_songs')||'[]');}catch(e){return[];}
}
function saveLikedSongs(songs){
    try{localStorage.setItem('austin_liked_songs',JSON.stringify(songs));}catch(e){}
}
...
function getLikedArtists(){
    try{return JSON.parse(localStorage.getItem('austin_liked_artists')||'[]');}catch(e){return[];}
}
function saveLikedArtists(artists){
    try{localStorage.setItem('austin_liked_artists',JSON.stringify(artists));}catch(e){}
}
...
function updateLikeButtons(){
    ...
    if(miniBtn){
        if(isLiked){
            miniBtn.innerHTML = '<i data-lucide="heart" class="w-4 h-4 text-white fill-white"></i>';
            miniBtn.classList.add('text-white');
        } else {
            miniBtn.innerHTML = '<i data-lucide="heart" class="w-4 h-4"></i>';
            miniBtn.classList.remove('text-white');
        }
    }

    if(fullBtn){
        if(isLiked){
            fullBtn.innerHTML = '<i data-lucide="heart" class="w-5 h-5 text-white fill-white"></i>';
            fullBtn.classList.add('bg-white/15', 'border-white/30');
            fullBtn.classList.remove('bg-black/50', 'border-white/20');
        } else {
            fullBtn.innerHTML = '<i data-lucide="heart" class="w-5 h-5 text-white"></i>';
            fullBtn.classList.remove('bg-white/15', 'border-white/30');
...
function getUserPlaylists(){
    try{
        var pls=JSON.parse(localStorage.getItem('austin_playlists')||'[]');
        var changed=false;
        ...
        if(changed){
            localStorage.setItem('austin_playlists',JSON.stringify(pls));
        }
        return pls;
    }catch(e){return[];}
}
function saveUserPlaylists(pls){try{localStorage.setItem('austin_playlists',JSON.stringify(pls));}catch(e){}}
...
function showToast(msg){
    ...
    var iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>';

    if (/gagal|belum|penuh|batal|error|tidak/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>';
    } else if (/hapus|dihapus/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>';
    } else if (/disukai|suka/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0 fill-white/90" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>';
    } else if (/timer|tidur|menit/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>';
    } else if (/acak|shuffle/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg>';
        ...
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>';
    } else if (/link|salin|clipboard/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>';
    } else if (/kecepatan/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>';
    } else if (/playlist|tersimpan/i.test(m)) {
        iconSvg = '<svg class="w-3.5 h-3.5 text-white/90 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>';
    }
...
function openSleepTimer() {
    ...
    if (S.sleepSecondsLeft > 0) {
        contentHtml = '<div class="text-center mb-6">' +
            ...
            '<button onclick="clearSleepTimer()" class="mt-4 px-6 py-2.5 rounded-full text-xs font-bold bg-white/10 text-white hover:bg-white/20 active:scale-95 transition-all">Batalkan Timer</button>' +
        '</div>';
    } else if (S.sleepEndWithTrack) {
        contentHtml = '<div class="text-center mb-6">' +
            ...
            '<button onclick="clearSleepTimer()" class="px-6 py-2.5 rounded-full text-xs font-bold bg-white/10 text-white hover:bg-white/20 active:scale-95 transition-all">Batalkan Timer</button>' +
        '</div>';
    } else {
...
function setPlaybackSpeed(speed) {
    ...
    try {
        localStorage.setItem('austin_playback_rate', speed);
    } catch(e) {}
    ...
    popup.innerHTML = '<div class="w-full max-w-sm rounded-3xl p-6 border border-white/10 glass-strong text-center" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div id="share-card-preview" class="p-6 rounded-2xl mb-6 flex flex-col items-center gap-4 relative overflow-hidden" ' +
            ...
            '<div class="border-t border-white/5 w-full pt-3 mt-1 flex items-center justify-center gap-1.5">' +
                ...
                '<span class="text-[10px] text-[#6b7280] tracking-wider font-semibold uppercase">AustinPlay Web App</span>' +
            '</div>' +
        '</div>' +
...
function triggerNativeShare() {
    ...
    if (navigator.share) {
        navigator.share({
            title: S.ct.title,
            text: 'Dengarkan ' + S.ct.title + ' - ' + S.ct.artist + ' di AustinPlay!',
            url: url
        }).catch(function() {});
    } else {
        copyShareLink();
...
function downloadShareCard() {
    ...
    img.onload = function() {
        ...
        ctx.fillText('DIDENGARKAN DI AUSTINPLAY', 300, 710);
        
        try {
            var dataUrl = canvas.toDataURL('image/png');
            var a = document.createElement('a');
            a.download = S.ct.title.replace(/[^a-zA-Z0-9]/g, '_') + '_austinplay.png';
            a.href = dataUrl;
            a.click();
