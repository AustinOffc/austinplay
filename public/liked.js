var Liked = {
    render() {
        ...
        if(liked.length > 0) {
            likedHtml = liked.map(function(s, i) {
                ...
                return '<div class="rounded-2xl '+cardBg+' p-2.5 flex items-center gap-3 active:scale-[0.98] transition-all duration-300 group shadow-lg shadow-black/25">'+
                    '<div onclick="PK(\'liked\','+i+')" class="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">'+
                        ...
                        '</div>'+
                    '</div>'+
                    '<button onclick="toggleLikeSong('+es(JSON.stringify(s)).replace(/"/g, '&quot;')+');Liked.render();" class="w-9 h-9 rounded-2xl bg-white/10 text-white border border-white/20 flex items-center justify-center shrink-0 hover:bg-white/20 active:scale-90 transition-all" title="Hapus dari Disukai">'+
            ...
            likedHtml = '<div class="text-center py-16 rounded-3xl bg-white/[0.04] border border-white/10 px-4"><div class="w-16 h-16 rounded-full bg-white/10 border border-white/20 flex items-center justify-center mx-auto mb-3"><i data-lucide="heart" class="w-8 h-8 text-white/80"></i></div><h3 class="text-white font-bold text-base mb-1">Belum ada lagu disukai</h3><p class="text-white/60 text-xs max-w-xs mx-auto">Klik ikon hati pada lagu favoritmu untuk menyimpannya di sini.</p></div>';
        }
        ...
        <div class="pt-8 pb-3.5 px-4 sticky top-0 z-30 border-b border-white/10 shadow-2xl transition-all flex justify-between items-center" style="background: linear-gradient(180deg, rgba(8, 9, 13, 0.4) 0%, rgba(8, 9, 13, 0.75) 100%), url('/banner.png') center/cover no-repeat; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">
            ...
            <div class="w-10 h-10 rounded-2xl bg-black/40 backdrop-blur-md border border-white/20 flex items-center justify-center text-white shadow-lg">
                <i data-lucide="heart" class="w-5 h-5 fill-current"></i>
            </div>
        </div>
        <div class="px-4 mt-4 space-y-3">
            ${liked.length > 0 ? `<div class="flex items-center justify-between mb-2"><span class="text-xs font-semibold text-white/60 uppercase tracking-wider">${liked.length} Lagu Tersimpan</span><button onclick="PK('liked',0)" class="text-xs text-white font-bold hover:underline flex items-center gap-1"><i data-lucide="play" class="w-3.5 h-3.5 fill-current"></i> Putar Semua</button></div>` : ''}
