...
window.addEventListener('appinstalled',function(){
    ...
    showToast('AustinPlay berhasil diinstall!');
});
    ...
    modal.innerHTML = '<div class="bg-[#121318] border border-white/15 rounded-2xl p-5 max-w-xs w-full text-center space-y-3 shadow-2xl relative" onclick="event.stopPropagation()">'+
        ...
        '<div class="space-y-1">'+
            ...
            '<p class="text-white/60 text-xs leading-relaxed">'+
                'Fitur Mode Offline khusus untuk aplikasi PWA. Silakan install AustinPlay ke layar utama terlebih dahulu.'+
            '</p>'+
        '</div>'+
    ...
    if(deferredInstallPrompt){
        ...
        deferredInstallPrompt.userChoice.then(function(choice){
            if(choice.outcome==='accepted') {
                try { localStorage.setItem('pwa_installed', 'true'); } catch(e){}
                isStandaloneApp = true;
                showToast('Menginstall AustinPlay...');
            }
            deferredInstallPrompt=null;
            var btn=document.getElementById('pwa-install-btn');
            if(btn)btn.classList.add('hidden');
...
function updateOfflineButtons() {
    ...
    if (fullBtn) {
        if (isSaved) {
            fullBtn.className = 'w-11 h-11 rounded-full bg-white/15 text-white border border-white/30 flex items-center justify-center active:scale-90 transition-all shrink-0 cursor-pointer shadow-md';
            ...
            songsHtml = offlineSongs.map(function(s, i) {
                ...
                return '<div class="flex items-center gap-3 p-2.5 rounded-2xl border '+cardBg+' active:scale-[0.98] transition-all duration-200 group shadow-md">'+
                    '<div onclick="PK(\'offline\','+i+')" class="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">'+
                        ...
                        '</div>'+
                    '</div>'+
                    '<button onclick="event.stopPropagation();saveTrackForOffline('+safeSongJson+');" class="w-8 h-8 rounded-full bg-white/5 hover:bg-white/15 text-white/50 hover:text-white border border-white/10 flex items-center justify-center shrink-0 active:scale-90 transition-all" title="Hapus dari Mode Offline PWA">'+
        ...
        <div class="pt-8 pb-3.5 px-4 sticky top-0 z-30 border-b border-white/10 shadow-2xl transition-all flex justify-between items-center bg-black/80 backdrop-blur-md">
            <div>
                <div class="flex items-center gap-2">
                    ...
                    <span class="px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase border ${isOnline ? 'border-white/40 text-white bg-white/10' : 'border-white/15 text-white/50 bg-white/5'}">${isOnline ? 'Online' : 'Offline'}</span>
                </div>
...
var App={
    navOpen:false,
    toggleNav(forceClose){
        var panel = gid('nav-expand-panel'), icon = gid('nav-fab-icon'), fab = gid('nav-fab');
        if(!panel || !icon) return;
        App.navOpen = forceClose === true ? false : !App.navOpen;
        if(App.navOpen){
            panel.classList.add('nav-expand-panel--open');
            icon.setAttribute('data-lucide','x');
            if(fab) fab.classList.add('nav-fab--active');
        } else {
            panel.classList.remove('nav-expand-panel--open');
            icon.setAttribute('data-lucide','chevron-up');
            if(fab) fab.classList.remove('nav-fab--active');
        }
        lucide.createIcons();
    },
    init(){
        ...
        <div class="fixed bottom-5 left-1/2 -translate-x-1/2 z-[170] flex flex-col items-center">
            <div id="nav-expand-panel" class="nav-expand-panel glass-dock rounded-[28px] py-2 px-2 mb-3 flex items-center gap-1 shadow-2xl">
                ...
                <button onclick="App.switch('search')" id="nav-search" class="nav-item group relative flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation py-1.5 px-2.5 rounded-2xl transition-all duration-300 active:scale-95">
                ...
                <button onclick="App.switch('library')" id="nav-library" class="nav-item group relative flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation py-1.5 px-2.5 rounded-2xl transition-all duration-300 active:scale-95">
                ...
                <button onclick="App.switch('offline')" id="nav-offline" class="nav-item group relative flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation py-1.5 px-2.5 rounded-2xl transition-all duration-300 active:scale-95">
                ...
                <button onclick="App.switch('liked')" id="nav-liked" class="nav-item group relative flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation py-1.5 px-2.5 rounded-2xl transition-all duration-300 active:scale-95">
                ...
                <button onclick="App.switch('dev')" id="nav-dev" class="nav-item group relative flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation py-1.5 px-2.5 rounded-2xl transition-all duration-300 active:scale-95">
            ...
            <button onclick="App.toggleNav()" id="nav-fab" class="nav-fab w-14 h-14 rounded-full glass-dock shadow-2xl flex items-center justify-center active:scale-90 transition-transform duration-300" title="Menu">
                <i data-lucide="chevron-up" id="nav-fab-icon" class="w-6 h-6 text-white transition-transform duration-300"></i>
            </button>
        </div>`;
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='AustinPlay',cover=toHDCover('', videoId),artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'AustinPlay',cover:toHDCover('', videoId),artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();resetLyricsUI(videoId);
            FullPlayer.open();loadTrack(S.ct);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='AustinPlay',cover=toHDCover('', videoId);
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','AustinPlay',toHDCover('', videoId));});
    },
    ...
    switch(t){
        // Collapse the expandable nav dock whenever a destination is chosen
        if(App.navOpen) App.toggleNav(true);
    ...
    showV2Popup() {
        // Popup versi baru dinonaktifkan
        return;
    }
};
App.init();Home.fetch();
...
function hideSplashScreen() {
    ...
    var minDuration = 1700; // lets the cinematic logo/wordmark reveal finish before dismissal
